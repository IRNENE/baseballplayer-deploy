...
useEffect(() => {
    ...
    // 更新搜索结果，但只有在搜索條件發生變化時才更新
    setSearchedProducts(searchQuery !== '' ? searchFilteredProducts : []);
}, [searchQuery, filteredProducts]);



    ///////////////////////////篩選分頁
    const [filteredCurrentPage, setFilteredCurrentPage] = useState(1); // 当前筛选后商品列表的当前页码
    const [filteredProductsPerPage] = useState(24); // 每页显示的筛选后商品数量

    // 计算总页数
    const filteredTotalPages = Math.ceil(filteredProducts.length / filteredProductsPerPage);

    // 根据当前页码计算当前页的筛选后商品列表
    const indexOfLastFilteredProduct = filteredCurrentPage * filteredProductsPerPage;
    const indexOfFirstFilteredProduct = indexOfLastFilteredProduct - filteredProductsPerPage;
    ...
    <Pagination
        ...
        onPrevClick={handleFilteredPrevPage}
        onNextClick={handleFilteredNextPage}
    />
                    ...
                    <div className="col-12 col-sm-10 mt-5">
                        ...
                        <div className={`col-3 msAutoReset xxx d-flex align-items-center mt-3  ${styles.xxx} ${styles.msAutoReset} `}>
                            ...
                            {filteredTotalPages > 0 ? (
                                <span className={`ppp1 me-1 ms-auto ${styles.ppp1} ppp1-mobile-align-left`}>共 {filteredProducts.length} 件商品</span>
                            ) : (
                                filteredTotalPages === 0 ? (
                                    <span className={`ppp1 me-1 ms-auto ${styles.ppp1} ppp1-mobile-align-left`}>共 0 件商品</span>
                                ) : (
                            ...
                            <select className={`form-select  rounded-0  ppx  ${styles.ppx} `} name="order" id="order" value={sortOption} onChange={handleSortChange} >
                                <option >排序</option>
                                
                                <option value="priceLowToHigh">價格低到高</option>
                                <option value="priceHighToLow">價格高到低</option>
                                <option value="3">最熱門</option>
                                    ...
                                    <ul className="pagination">
                                        ...
                                        {searchQuery !== '' ? (
                                            ...
                                            Array.from({ length: filteredTotalPages }, (_, index) => (
                                                <li key={index} className={`page-item ms-2 ${filteredCurrentPage === index + 1 ? 'active' : ''}`}>
                                                    <a className="page-link text-black" href="#" onClick={() => handleFilteredPageClick(index + 1)}>{index + 1}</a>
                                                </li>
                                            ))
                                        )}
                                        <li className="page-item ms-2">
