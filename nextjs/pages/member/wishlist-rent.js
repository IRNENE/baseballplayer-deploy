...
export default function Wishlist() {
    ...
    const currentProducts = wishlist.slice(indexOfFirstProduct,
        indexOfLastProduct);  // 對應 一頁有幾筆
    ...
    const fetchFavorites = () => {
        ...
        fetch(`http://localhost:3005/api/member-center/wishlist-rent/${userData.id}`)
            .then(response => {
                // 檢查 HTTP 狀態碼
                if (!response.ok) {
    ...
    const fetchURL = async () => {
        let url = `http://localhost:3005/api/member-center/wishlist-rent/${userData.id}?`;
        const queryParams = [];
        if (sortOrder) {
    ...
    const handleToggleFav = (rentId) => {
        ...
        fetch(`http://localhost:3005/api/wishlist-rent/${rentId}`,
            ...
            .then((res) => {
                if (res.ok) {
                    // 如果移除收藏成功，更新收藏狀態
                    setFavoriteRentIds(prevIds => prevIds.filter(id => id !== rentId))
            ...
            <div className={`${styles.containerRWD} container`}>
                <div className="row">
                    ...
                    <div className={`${styles.tabsMargin} col-12  col-sm-9 pt-3`}>
                        <ul className={`${styles.ulBorder} list-unstyled d-flex justify-content-around text-center`}>
                            ...
                            <div className={`${styles.active1}`}> <li className='pb-2'><Link className={styles.noBottom} href="./wishlist-rent">租借收藏</Link></li></div>
                ...
                <div className="row">
                    ...
                    <div className={`${styles.rwdMargin} col-12 col-sm-9 py-2`}>
                        ...
                        dataToShow.map((rent) => (
                            <div key={rent.id} className={` me-2 my-2 ${styles.box3}`}>
                                <figcaption className={styles.avatar}>
                                    <a href={`/rent/detail?pid=${rent.product_id}`}>
                                        <img
                                ...
                                <a href={`/rent/detail?pid=${rent.product_id}`}>
                                    <h6 className={`${styles2.name} ${styles2.singleEllipsis1}`}>{rent.name}</h6>
                                </a>
                                <div className={`${styles2.box4} mt-4`}>${Math.round(rent.price * 0.8)}</div>
                                <div className={styles2.bbox}>
                                    <p className={styles2.box5}>${rent.price}</p>
                                    <div onClick={() => handleToggleFav(rent.product_id)} className={`${styles2.iconContainer}`} style={{ fontSize: '1.2rem', color: 'red' }}>
                                        <FaHeart />
                                    </div>
                                </div>
                            </div>
