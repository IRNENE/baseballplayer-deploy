...
export default function CouponLayout() {
  ...
  // 追踪当前是显示“未使用”还是“已使用”的优惠券。
  const [currentStatus, setCurrentStatus] = useState('unused') // 'unused' 或 'used'
  ...
  const handleFilterChange = (isUsed) => {
    ...
    setCurrentStatus(isUsed === 0 ? 'unused' : 'used')
  ...
  console.log('測試filteredData', filteredData)
      ...
      <div className="container">
        ...
        <div className={`row`}>
          ...
          <div className={`col-12 col-sm-9`}>
            ...
            {/* 優惠卷輸入 & 排序 */}
            <div className={`row ${styles['coupon-space']}`}>
              <div className={`d-none d-sm-block col-sm-4`}></div>
              <div className={`col-12  col-sm-6`}>
                <div className={`${styles['coupon-input-box']} mb-3`}>
                  <form
                  ...
                  <button
                    type="button"
                    className={`btn btn-primary ${styles['code-btn']}`}
              ...
              {/* <div
                className={`d-none d-sm-block col-sm-1`}
                style={{ width: '1px' }}
              ></div> */}

              <div className={`col-8 d-block d-sm-none `}></div>
              <div className={`col-4 col-sm-2 `}>
                {/* 優惠卷排序按鈕 */}
                <select
                  id="sort"
                  className={`w-100 ${styles['select-sort']} form-select`}
                ...
                <div className={styles['coupon-amount']}>
                  <h6 className="mb-1">
                    ...
                    {currentStatus === 'unused' ? (
                      <>
                        共有<span>{filteredData.length}</span>
                        張優惠卷可使用，一次限用一張結帳
                      </>
                    ) : (
                      <>
                        已使用<span>{filteredData.length}</span>張優惠卷
                      </>
                    )}
                  </h6>
                </div>
