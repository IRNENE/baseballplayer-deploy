...
import styles3 from '@/pages/course/course.module.css'
...
import SuperSimple from '@/pages/course/price'
// import Example from '@/pages/course/Example' // 導入 Example 組件
import ModalComponent from '@/pages/course/modal'
import Sorting from '@/pages/course/sorting'
...
export default function Wishlist() {
    ...
    const fetchFavorites = () => {
        ...
        fetch(`http://localhost:3005/api/member-center/wishlist-course/${userData.id}`)
            .then(response => {
                // 檢查 HTTP 狀態碼
                if (!response.ok) {
    ...
    useEffect(() => {
        fetchFavorites()
    }, [userData.id]) //useEffect 的依賴項(會員id切換才會重新渲染)
    ...
    const fetchURL = async () => {
        let url = `http://localhost:3005/api/member-center/wishlist-course/${userData.id}?`;
    ...
    const handleToggleFav = (courseId) => {
        // 向後端發送 POST 請求以移除收藏
        fetch(`http://localhost:3005/api/wishlist-course/${courseId}`, {
            method: 'POST',
            credentials: 'include'
        })
            .then((res) => {
                if (res.ok) {
                    // 如果移除收藏成功，更新收藏狀態
                    setFavoriteCourseIds(prevIds => prevIds.filter(id => id !== courseId));
                ...
                <div className="row">
                    ...
                    <div className={`${styles.tabsMargin}  col-12  col-sm-9 pt-3`}>
                        <ul className={`${styles.ulBorder} list-unstyled d-flex justify-content-around text-center`}>
                            <div style={{ width: '352px' }}> <li className='pb-2'><Link className={styles.noBottom} href="./wishlist">商品收藏</Link></li> </div>
                            <div style={{ width: '352px' }}> <li className='pb-2'><Link className={styles.noBottom} href="./wishlist-rent">租借收藏</Link></li></div>
                            <div className={`${styles.active1}`}> <li className='pb-2'><Link className={styles.noBottom} href="./wishlist-course">課程收藏</Link></li></div>
                        </ul>
                    </div>
                </div>
                <div className="row">
                    ...
                    <div className={`${styles.rwdMargin} col-12 col-sm-9 py-2`}>
                        <div className="d-flex  align-items-end pb-3">
                            <span className={`${styles.perpageColor} ms-auto`} >共收藏 {wishlist.length} 堂</span>
                        ...
                        {dataToShow.length === 0 ? (
                            ...
                            dataToShow.map((course) => (
                                <div
                                    key={course.id}
                                    className={`course-card-sun d-flex  ${styles3.CourseCardSun}`}
                                >
                                    {' '}
                                    <div className={`${styles3.CourseCardImg}`}>
                                        <a href={`/course/${course.product_id}`}>
                                            <img src={`/images/course/${course.photo}`} alt="" />
                                        </a>

                                    </div>
                                    <div
                                        className={`courseCardTextSun ${styles3.CourseCardTextSun}`}
                                    >
                                        <p className="fw-bold">{course.type}</p>
                                        <a href={`/course/${course.product_id}`}>
                                            <h3 className={`teacherText ${styles3.courseNameText}`}>
                                                {course.name}
                                            </h3>
                                        </a>
                                        <p>教練:{course.t_name}</p>
                                        <p>
                                            時間:
                                        ...
                                        <div
                                            className={`description ${styles3.description}`}
                                    ...
                                    >
                                        ...
                                        <div
                                            className={`d-flex justify-content-center text-center ${styles3.CourseCardFavorite}`}
                                            onClick={() => handleToggleFav(course.product_id)} style={{ fontSize: '1.2rem', color: 'red' }}
                                        >
                                            <FaHeart />
                                        </div>
                                        <div className={`mt-auto  ${styles3.CourseCardPrice}`}>
                                            <p>${course.price}</p>
                                        </div>
                                    </div>
                                </div>
