...
router.get('/user/:id/', authenticate, async (req, res) => {
  ...
  const sql = `SELECT * FROM favorite WHERE user_id = ? AND class_id = 'R'`
...
// 將商品添加到收藏列表
router.post('/:rentId', authenticate, async (req, res) => {
  const rentId = req.params.rentId
  const userId = req.user.id

  try {
    // 檢查商品是否已經在收藏列表中
    const checkSql = `SELECT * FROM favorite WHERE user_id = ${userId} AND product_id = ${rentId} AND class_id = 'R'`
    const [existingFavorite] = await db.query(checkSql)
    // console.log(existingFavorite)
    if (existingFavorite.length > 0) {
      const deleteSql = `DELETE FROM favorite WHERE user_id = ${userId} AND product_id = ${rentId}`
    ...
    // 將商品添加到收藏列表
    const insertSql = `INSERT INTO favorite (user_id, product_id , class_id) VALUES (${userId}, ${rentId} , 'R')`
