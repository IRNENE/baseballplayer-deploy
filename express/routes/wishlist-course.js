...
router.post('/:courseId', authenticate, async (req, res) => {
  ...
  try {
    // 檢查商品是否已經在收藏列表中
    const checkSql = `SELECT * FROM favorite WHERE user_id = ${userId} AND product_id = ${courseId} AND class_id = 'C'`
    const [existingFavorite] = await db.query(checkSql)
    // console.log(existingFavorite)
    if (existingFavorite.length > 0) {
      const deleteSql = `DELETE FROM favorite WHERE user_id = ${userId} AND product_id = ${courseId}`
    ...
    // 將商品添加到收藏列表
    const insertSql = `INSERT INTO favorite (user_id, product_id , class_id) VALUES (${userId}, ${courseId} , 'C')`
